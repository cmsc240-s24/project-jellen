...
TEST_CASE("Testing Cart Class") 
{
    ...
    SUBCASE("Testing the Cart Constructor") 
    {
        ...
        Cart testCart(json::load(R"({"cartID":"1","movies":[{"id":"1","title":"The Grand Budapest Hotel","name":"Wes Anderson"},{"id":"2","title":"Soul","name":"Director2"}]})"));

        // Check that the constructor properly loaded the values.
        CHECK(testCart.getID() == "1");
    ...
    SUBCASE("Testing updateFromJson Method") 
    {
        ...
        json::rvalue updateJson = json::load(R"({"cartID":"2","movies":[{"id":"3","title":"Whiplash","name":"Damien Chazelle"},{"id":"4","title":"The Cheetah Girls 2","name":"Kenny Ortega"}]})");

        // Update the Cart with the updateFromJson method. 
        testCart.updateFromJson(updateJson);

        // Check the updated values.
        CHECK(testCart.getID() == "2");
        CHECK(testCart.getMovies().size() == 2);
